...
interface Props { onBack?: () => void; initialMode?: 'login' | 'register'; forceReset?: boolean; onResetDone?: () => void; }

export default function AuthScreen({ onBack, initialMode = 'register', forceReset = false, onResetDone }: Props) {
  const { dispatch } = useUser();
  const [mode, setMode]       = useState<Mode>(forceReset ? 'reset' : initialMode);
  ...
  async function handleForgotSubmit(e: React.FormEvent) {
    ...
    if (!isSupabaseConfigured || !supabase) { setError("La récupération de mot de passe n'est pas disponible hors ligne."); return; }
    setLoading(true);
  ...
  const titles: Record<Mode, { h1: string; sub: string }> = {
    register: { h1: 'Créer un compte', sub: 'Commencez votre apprentissage du russe gratuitement.' },
    login:    { h1: 'Bon retour !', sub: 'Reprenez là où vous en étiez.' },
    forgot:   { h1: 'Mot de passe oublié', sub: 'On vous envoie un lien pour le réinitialiser.' },
    reset:    { h1: 'Nouveau mot de passe', sub: 'Choisissez un mot de passe pour votre compte.' },
  };
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(2rem, 5vw, 4rem) clamp(1.25rem, 5vw, 2rem)' }}>
        <div className="animate-fade-in" style={{ width: '100%', maxWidth: '400px' }}>
          ...
          <div style={{ marginBottom: '2.5rem' }}>
            ...
            <p style={{ color: '#737373', fontSize: '0.92rem', margin: 0 }}>{titles[mode].sub}</p>
          </div>
          ...
          {mode === 'reset' && (
            resetDone ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', textAlign: 'center', padding: '1rem 0' }}>
                ...
                <p style={{ color: '#4A4A4A', fontSize: '0.92rem', margin: 0 }}>
                  Mot de passe mis à jour. Vous pouvez continuer.
                </p>
                <button
                  onClick={() => onResetDone?.()}
                  style={{ background: '#0A0A0A', color: 'white', border: 'none', padding: '0.8rem 1.75rem', borderRadius: '10px', fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer' }}
                >
                  Continuer
                </button>
              </div>
              ...
              <form onSubmit={handleResetSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 600, color: '#0A0A0A', marginBottom: '5px' }}>Nouveau mot de passe</label>
                  <div style={{ position: 'relative' }}>
                    <input type={showPwd ? 'text' : 'password'} value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="••••••••"
                      style={{ ...inputStyle(pwdFocus), paddingRight: '3rem' }}
                      onFocus={() => setPwdFocus(true)} onBlur={() => setPwdFocus(false)} autoComplete="new-password" autoFocus />
                ...
                <button type="submit" disabled={loading}
                  style={{ background: '#0A0A0A', color: 'white', border: 'none', padding: '0.9rem', borderRadius: '10px', fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: '0.95rem', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.65 : 1 }}
                >
            ...
            sent ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', textAlign: 'center', padding: '1rem 0' }}>
                ...
                <button onClick={() => { setMode('login'); setSent(false); }}
                  style={{ background: 'none', border: 'none', fontSize: '0.85rem', fontWeight: 700, color: '#0A0A0A', cursor: 'pointer', textDecoration: 'underline' }}>
              ...
              <form onSubmit={handleForgotSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div>
                  ...
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="vous@exemple.com"
                    style={inputStyle(emailFocus)} onFocus={() => setEmailFocus(true)} onBlur={() => setEmailFocus(false)} autoComplete="email" autoFocus />
                </div>
                {error && (
                  <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '8px', padding: '0.65rem 0.9rem', fontSize: '0.83rem', color: '#DC2626' }}>{error}</div>
                )}
                ...
                <button type="button" onClick={() => { setMode('login'); setError(''); }}
                  style={{ background: 'none', border: 'none', fontSize: '0.85rem', color: '#737373', cursor: 'pointer', marginTop: '4px' }}>
                  ← Retour à la connexion
                </button>
              </form>
            )
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '5px' }}>
                    <label style={{ fontSize: '0.78rem', fontWeight: 600, color: '#0A0A0A' }}>Mot de passe</label>
                    {mode === 'login' && (
                      <button type="button" onClick={() => { setMode('forgot'); setError(''); }}
                        style={{ background: 'none', border: 'none', fontSize: '0.76rem', color: '#737373', cursor: 'pointer', textDecoration: 'underline', textDecorationStyle: 'dotted' }}>
                        Mot de passe oublié ?
                      </button>
                    )}
                  </div>
