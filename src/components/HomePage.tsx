...
import { ArrowRight, BookOpen, Layers, PenLine, Volume2, Zap, Star, Coffee } from 'lucide-react';
import { AppLogo, BearMark } from './Logo';

function XLogo({ size = 16 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M18.24 2H21.5l-7.3 8.34L22.8 22h-6.77l-5.3-6.94L4.6 22H1.34l7.8-8.92L1.2 2h6.94l4.79 6.34zm-1.18 18h1.83L7.02 3.9H5.05z" />
    </svg>
  );
}
...
function GithubLogo({ size = 16 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M12 .5C5.73.5.5 5.73.5 12c0 5.09 3.29 9.4 7.86 10.93.57.1.78-.25.78-.55 0-.27-.01-1.16-.02-2.11-3.2.7-3.88-1.36-3.88-1.36-.52-1.33-1.28-1.68-1.28-1.68-1.04-.72.08-.7.08-.7 1.16.08 1.77 1.19 1.77 1.19 1.03 1.77 2.71 1.26 3.37.96.1-.75.4-1.26.73-1.55-2.55-.29-5.24-1.28-5.24-5.69 0-1.26.45-2.28 1.19-3.09-.12-.29-.52-1.46.11-3.04 0 0 .97-.31 3.18 1.18a11 11 0 0 1 5.79 0c2.2-1.49 3.17-1.18 3.17-1.18.64 1.58.24 2.75.12 3.04.74.81 1.18 1.83 1.18 3.09 0 4.42-2.69 5.39-5.25 5.68.41.36.78 1.06.78 2.15 0 1.55-.01 2.8-.01 3.18 0 .3.2.66.79.55A10.52 10.52 0 0 0 23.5 12c0-6.27-5.23-11.5-11.5-11.5z" />
    </svg>
  );
}
...
export default function HomePage({ onStart, onLogin }: Props) {
  ...
  return (
    <div ref={containerRef} style={{ height: '100%', overflowY: 'auto', background: '#FAFAF8', fontFamily: 'var(--font-ui)', color: '#0A0A0A' }}>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ borderTop: '1px solid #EBEBEB', padding: '1.75rem clamp(1.25rem, 6vw, 4rem)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1.25rem', background: 'white' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <a href="https://github.com/PatrickLoic-dev/Davai" target="_blank" rel="noreferrer"
              aria-label="Le projet Davai sur GitHub"
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '32px', height: '32px', borderRadius: '50%', color: '#5A5A5A', border: '1px solid #E8E8E8', transition: 'color 0.15s, border-color 0.15s' }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#0A0A0A'; (e.currentTarget as HTMLElement).style.borderColor = '#0A0A0A'; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = '#5A5A5A'; (e.currentTarget as HTMLElement).style.borderColor = '#E8E8E8'; }}
            ><GithubLogo size={16} /></a>

            <a href="https://x.com/KewellLoic" target="_blank" rel="noreferrer"
              aria-label="Patrick Loïc sur X"
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '32px', height: '32px', borderRadius: '50%', color: '#5A5A5A', border: '1px solid #E8E8E8', transition: 'color 0.15s, border-color 0.15s' }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#0A0A0A'; (e.currentTarget as HTMLElement).style.borderColor = '#0A0A0A'; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = '#5A5A5A'; (e.currentTarget as HTMLElement).style.borderColor = '#E8E8E8'; }}
            ><XLogo size={14} /></a>

            <a href="https://buymeacoffee.com/patrickloic" target="_blank" rel="noreferrer"
              style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '0.4rem 0.85rem', borderRadius: '100px', background: '#FFDD00', color: '#0A0A0A', textDecoration: 'none', fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: '0.78rem', transition: 'opacity 0.15s' }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.opacity = '0.85'}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.opacity = '1'}
            ><Coffee size={14} /> Buy me a coffee</a>
          </div>
        </div>
      </footer>
